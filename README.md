...
$p(t)=\sum_{i=0}^nd_iN_{i,k}(t)$, 其中，$N_{i,k}(t)$是k次B样条基函数。

## 拟合方法
可以将曲线公式写成$Ax=b$形式，$x$为未知参数。使用最小二乘法求解：$x=(A^TA)^{-1}A^Tb$

## 评估方法
得到拟合函数后，在$t$的$[0, 1]$区间中均匀选取$K$个采样点进行预测(在实验中$K=10000$)。对每一个预测出来的点分别求$\theta$值，代入公式$r = 0.5 + 0.2sin5\theta$, 并从极坐标转为平面直角坐标系，作为目标值。
...
当$M=k+1$时为曲线插值；当$M>k+1$时为曲线拟合；当$M<=k$时，无法拟合曲线(记为x)。

在方程次数不变的情况下，随着采样点数量增多，误差减小。M=4, k=3时已经能很好地拟合图形了。
...
- 使用梯度
使用均匀采样，均匀参数法，3次多项式拟合，使用梯度信息，一个区间的采样点数量为M。
...
M=4|0.00459|0.01220
M=6|0.00439|0.00987
M=10|0.00388|0.00861

使用梯度信息后，虽然曲线的端点$C^1$连续，但拟合效果反而变差了。原因是函数$G$的切矢量变化过大。
